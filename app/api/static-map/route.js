...
async function GET(req) {
    try {
        ...
        if (!lat || !lon) {
            return server_1.NextResponse.json({ error: 'lat and lon required' }, { status: 400 });
        }
        const token = process.env.MAPBOX_SECRET_TOKEN || process.env.NEXT_PUBLIC_MAPBOX_TOKEN;
        if (!token) {
            return server_1.NextResponse.json({ error: 'Mapbox token not configured on the server' }, { status: 500 });
        }
        ...
        if (!resp.ok) {
            ...
            const osmUrl = `https://staticmap.openstreetmap.de/staticmap.php?center=${encodeURIComponent(`${lat},${lon}`)}&zoom=${z}&size=${w}x${h}&markers=${encodeURIComponent(`${lat},${lon},red-pushpin`)}`;
            ...
            if (osmResp.ok) {
                ...
                return new server_1.NextResponse(arrayBuffer, {
                    status: 200,
                    headers: { 'Content-Type': contentType, 'Cache-Control': 'public, max-age=3600' },
                });
            }
            const text = await resp.text();
            return server_1.NextResponse.json({ error: 'Mapbox fetch failed', detail: text }, { status: 502 });
        }
        ...
        return server_1.NextResponse.json({ error: 'unexpected', message: String(err) }, { status: 500 });
    }
}
